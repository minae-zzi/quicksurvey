...
const Pannel = () => {
  ...
  const blet2 = (<span className='pr-2 align-middle	'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  ...
  return (
    <div className="break-keep">
      ...
      <div className=''>
        ...
        <a name="section1"></a>
        <div className='pt-10 pb-10 p-4 lg:p-0 lg:h-screen flex flex-col justify-items-center items-center justify-center text-center'>
          <p className='text-4xl text-center lg:text-7xl lg:pb-8 pb-4 font-bold'>한국리서치 <span className='text-c1'>MasterSample</span>이란?</p>
          <p className='brnone'>한국리서치는 자발적인 조사 참여 의향을 가진 약 92만명의 자체 Master Sample® 을 보유하고 있습니다.<br />
통계적 대표성을 확보한 Master Sample® 을 활용하여 다양한 주제의 조사에서 적격응답자를 신속하게 확보할 수 있으며, 조사 결과의 정확성을 높일 수 있습니다.</p>

          <div className='grid lg:grid-flow-col lg:gird-rows-2 gap-8 w-11/12 lg:w-8/12 pt-10'>
            <div className='shadow-lg rounded-2xl p-8 w-full'>
              <p className='text-2xl font-bold pb-4 text-left'>성별</p>
              <img src={process.env.PUBLIC_URL + '/images/pannel_chart.png'} alt="chart" className='w-full' />
            </div>
            <div className='shadow-lg rounded-2xl p-8 w-full'>
              <p className='text-2xl font-bold pb-4 text-left'>연령별</p>
              <ul className='font-bold text-base grid lg:gap-3 lg:grid-cols-2 text-left items-center'>
            ...
            <p className='text-2xl font-bold pb-4 text-left relative'>지역별 <span className='lg:absolute lg:inline lg:text-lg text-base block right-0'>전체 Master Sample <span className='text-c1'>920,707명</span></span></p>
          ...
          {/* 특수패널 */}
          <a name="section2"></a>
          <div className='pt-10 pb-10 p-4 lg:p-0 lg:h-screen flex flex-col text-center justify-center items-center bg-zinc-50'>
          <p className='text-4xl text-center lg:text-7xl lg:pb-8 pb-4 font-bold'>한국리서치 <span className='text-c1'>특수 패널</span></p>
          <p className='brnone pb-14'>이 외에도 한국리서치는 특수 패널을 구축하여 보다 빠르고 정확하게 적격응답자를 찾을 수 있습니다</p>

            <div className='flex flex-col gap-6 lg:flex-row lg:gap-12 lg:w-7/12 w-11/12 justify-center lg:min-w-[1300px]'>
            ...
            <div className='flex flex-col lg:flex-row justify-start text-left items-center mt-8 lg:min-w-[1300px] lg:w-7/12 w-10/12'>
              <div className='w-full lg:w-3/12 bg-c1 text-white p-4 font-bold text-xl text-center rounded-full'>확대예정패널</div>
              ...
              <div className='bg-white shadow-xl p-4 mt-4 lg:mt-0 lg:pl-12 rounded-xl lg:rounded-full w-full border-l-2 border-r-2 border-c1'>
                <ul className='flex flex-col lg:flex-row lg:items-center lg:gap-10 gap-2'>
                  <li className='flex justify-center font-bold text-lg'>{blet2} 70대 이상 시니어 패널</li>
                  <li className='flex justify-center font-bold text-lg'>{blet2} 자녀+부모 쌍 패널</li>
                  <li className='flex justify-center font-bold text-lg'>{blet2} 국내 거주 외국인 패널</li>
                  <li className='flex justify-center font-bold text-lg'>{blet2} 다문화 가구 패널</li>
                  <li className='flex justify-center font-bold text-lg'>...</li>
                </ul>
              </div>
            </div>
          ...
          {/* 특장점 */}
          <a name="section3"></a>
          <div className='lg:h-screen flex flex-col items-center justify-center pt-14 pb-14 lg:p-0'>
            <p className='text-4xl text-center lg:text-7xl lg:pb-14 pb-4 font-bold'>한국리서치 MasterSample의 <span className='text-c1'>특장점은?</span></p>

            <div className='lg:w-7/12 lg:min-w-[1300px] grid gap-4 w-10/12 lg:grid-cols-4 lg:gap-8 justify-center items-start'>
 

                {
                  merit.map((item, i)=>{
                    return(
                      <div className='rounded-xl bg-zinc-50 p-12 pt-10 pb-10 flex flex-col items-center text-center lg:min-h-[320px]'>
                        <p>{item.ico}</p>
                        <p className='font-bold text-xl lg:whitespace-pre-line pb-2 pt-2'>{item.title}</p>
                        <p className='text-zinc-800 lg:whitespace-pre-line'>{item.content}</p>
                      </div>
                    )
                  })
                }
          ...
          {/* 패널관리체계 */}
          <a name="section4"></a>
          <div className='lg:h-screen flex flex-col items-center justify-center pt-14 pb-14 lg:p-0' style={{backgroundColor:'rgba(249, 238, 234, 0.40)'}}>
            <p className='text-4xl text-center lg:text-7xl lg:pb-10 pb-4 font-bold pr-4 pl-4'>한국리서치 MasterSample의 <span className='text-c1'>패널 관리 체계</span></p>
            <p className='brnone text-lg text-center leading-tight pl-14 pr-14'>안정적인 조사 운영 및 자료 신뢰도를 높이기 위해 Master Sample 등급 제도 등 엄격한 관리 시스템을 운영하고 있습니다.</p>

            <div className='lg:w-7/12 lg:min-w-[1300px] grid gap-2 w-10/12 lg:grid-cols-5 lg:gap-8 justify-center items-start pt-4 lg:pt-14'>
                
                {
                  hrcsystem.map((item, i)=>{
                    return(
                      <div className='flex flex-col justify-center items-center mt-4 lg:mt-0'>
                        <p className='rounded-full font-bold text-white p-4 bg-c2 text-lg leading-tight w-full text-center lg:whitespace-pre-line'>{item.title}</p>
                        ...
                        <div className='flex items-center rounded-xl p-8 pl-6 pr-6 text-center font-bold text-lg leading-tight border-t-2 border-c2 bg-white shadow-xl -mt-4 lg:min-h-[150px]'>{item.content}</div>
                      </div>
                    )
                  })
            ...
            <div className='flex flex-col lg:flex-row lg:justify-between items-center lg:w-7/12 lg:min-w-[1300px] pt-14'>
              <div className='flex flex-col justify-center items-center'>
                ...
                <p className='font-bold text-4xl'>등급제도</p>
              </div>
              <div className='flex justify-start w-10/12 relative'>
                <ul className='flex flex-col gap-2 font-bold text-base lg:text-lg mt-4 lg:mt-0'>
                  <li className='flex items-center lg:items-center'><span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>1</span> <span className="lg:w-[120px] pr-2">신규 패널</span> <span className="text-zinc-500">신규 가입 패널</span></li>
                  <li className='flex flex-col lg:flex-row'>
                    <div className='flex items-center'>
                      <span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>2</span> 
                      <span className="lg:w-[120px] pr-2">불성실 패널 </span>
                      <span className="text-zinc-500">1,2,3회 불성실 응답자</span> 
                    </div>
                  <span className='flex flex-row bb lg:absolute right-0 text-sm p-2 pl-4 pr-4 mt-2 lg:mt-0 rounded-full text-c1 border border-c1 leading-tight text-center'>1,2회 경고 이메일 보냄, 3회 시 Master Sample 해지</span></li>
                  <li className='flex items-center'><span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>3</span> <span className="lg:w-[120px] pr-2">활성 패널</span> <span className="text-zinc-500">현재 진행 중인 응답자</span> </li>
                  <li className='flex flex-col lg:flex-row'>
                    <div className='flex items-center'>
                      <span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>4</span> 
                      <span className="lg:w-[120px] pr-2">휴면 패널</span> <span className="text-zinc-500">1년간 접속이 없는 응답자 </span> 
                    </div>
                    <span className='lg:absolute right-0 text-sm p-2 pl-4 pr-4 mt-2 lg:mt-0 rounded-full text-c1 border border-c1 leading-tight text-center'>변경 확인 후 대응</span></li>
                  <li className='flex flex-col lg:flex-row'>
                    <div className='flex items-center'>
                      <span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>5</span> 
                      <span className="lg:w-[120px] pr-2">불량 패널</span> <span className="text-zinc-500">비 매너, 누리꾼, 불량 응답자 </span> 
                    </div>
                    <span className='lg:absolute right-0 text-sm p-2 pl-4 pr-4 mt-2 lg:mt-0 rounded-full text-c1 border border-c1 leading-tight text-center'>Master Sample 해지</span></li>
                  <li className='flex flex-col lg:flex-row'>
                  <div className='flex items-center'>
                    <span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>6</span> 
                    <span className="lg:w-[120px] pr-2">탈퇴 희망자</span> <span className="text-zinc-500">본인이 탈퇴를 원하는 자</span> 
                  </div>
                    <span className='lg:absolute right-0 text-sm p-2 pl-4 pr-4 mt-2 lg:mt-0 rounded-full text-c1 border border-c1 leading-tight text-center'>Master Sample 해지</span></li>
                  <li className='flex flex-col lg:flex-row'>
                  <div className='flex items-center'>
                    <span className='flex items-center justify-center w-8 h-8 bg-c1 text-white fon-bold rounded-full mr-2'>7</span> 
                    <span className="lg:w-[120px] pr-2">과다 참여 패널</span> <span className="text-zinc-500">연 24회 이상, 월 3회 이상 참여 제한 패널</span> 
                    </div>
                    <span className='lg:absolute right-0 text-sm p-2 pl-4 pr-4 mt-2 lg:mt-0 rounded-full text-c1 border border-c1 leading-tight text-center'>설문 전달 축소</span></li>
                </ul>
                <ul>
                  <li></li>
                </ul>
